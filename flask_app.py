...
def generate(nombreimagen,condicionletra):
    ...
    while True:
          ...
          if results.multi_hand_landmarks is not None:
            # Accediendo a los puntos de referencia, de acuerdo a su nombre   
                ...
                res2="nada"
                condicionletra(dedos, frame,pw,res2)
                etiqueta=condicionletra(dedos, frame,pw,res2)
...
@app.route('/letraB')
def letraB():
    return render_template('letraB.html') 
...
def video_feed():
     return Response(generate(nombreimagen,condicionales.condicionalesLetrasA),
          mimetype = "multipart/x-mixed-replace; boundary=frame")
@app.route("/video_feed_B")
def video_feed_B():
     return Response(generate(nombreimagen,condicionales.condicionalesLetrasB),
          mimetype = "multipart/x-mixed-replace; boundary=frame")
@app.route('/download')
def Download_File():
    print(nombreimagen)
